...
import { Edit2, X, CreditCard } from 'lucide-react';
import { updateBilling } from '@/lib/actions';
import { SubmitButton } from './SubmitButton';
import { BillingEntry } from '@/lib/types';

export function EditPaymentDialog({ payment }: { payment: BillingEntry }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" dir="rtl">
            <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 text-right">
                <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-2xl flex items-center justify-center">
                            <CreditCard size={20} />
                        </div>
                        <h2 className="text-2xl font-black text-gray-900">עריכת תשלום</h2>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                required
                                className="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 font-bold text-gray-900"
                                defaultValue={payment.monthRef}
                            />
                        </div>
                    </div>

                    <div className="pt-4 text-center">
                        <SubmitButton>עדכון תשלום</SubmitButton>
                    </div>
                </form>
            </div>
        </div>
